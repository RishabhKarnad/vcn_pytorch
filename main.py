...
def auroc(model, ground_truth, num_samples=1000):
    ...
    gt = utils.adj_mat_to_vec(torch.from_numpy(
        ground_truth).unsqueeze(0), model.num_nodes).numpy().squeeze()
    ...
    with torch.no_grad():
        while i < num_samples:
            ...
            samples.append(model.graph_dist.sample(
                [curr]).cpu().numpy().squeeze())
            i += curr
...
def full_kl_and_hellinger(model, bge_train, g_dist, device):
    ...
    bs = 100000
    all_adj = utils.all_combinations(
        model.num_nodes, return_adj=True).astype(np.float32)
    all_adj_vec = utils.all_combinations(
        model.num_nodes, return_adj=False).astype(np.float32)
    ...
    with torch.no_grad():
        for tt in range(0, len(all_adj), bs):
            log_posterior_graph[tt:tt+bs] = bge_train.log_marginal_likelihood_given_g(
                w=torch.tensor(all_adj[tt:tt+bs]).to(device)).cpu()  # Unnormalized Log Probabilities
            log_prob_model[tt:tt+bs] = model.graph_dist.log_prob(torch.tensor(
                all_adj_vec[tt:tt+bs]).to(device).unsqueeze(2)).cpu().squeeze()
        for tt in range(len(all_adj)):
            log_prob_g[tt] = g_dist.unnormalized_log_prob(g=all_adj[tt])
    graph_p = torch.distributions.categorical.Categorical(
        logits=log_posterior_graph + log_prob_g)
    graph_q = torch.distributions.categorical.Categorical(
        logits=log_prob_model)
    hellinger = (1./np.sqrt(2)) * torch.sqrt((torch.sqrt(graph_p.probs) -
                                              torch.sqrt(graph_q.probs)).pow(2).sum())
...
def train(model, bge_train, optimizer, baseline, batch_size, e, device):
    ...
    optimizer.zero_grad()
    # TODO: Check if additional entropy regularization is required
    likelihood, kl_graph, log_probs = model(batch_size, bge_train, e)
...
def load_model(args):
    if not args.no_autoreg_base:
        graph_dist = autoreg_base.AutoregressiveBase(
            args.num_nodes, device=args.device, temp_rsample=0.1).to(args.device)
    else:
        graph_dist = factorised_base.FactorisedBase(
            args.num_nodes, device=args.device, temp_rsample=0.1).to(args.device)
    ...
    model = vcn.VCN(num_nodes=args.num_nodes, graph_dist=graph_dist, sparsity_factor=args.sparsity_factor,
                    gibbs_temp_init=args.gibbs_temp_init, gibbs_update=gibbs_update).to(args.device)
...
def main(args):
    ...
    if args.num_nodes <= 4:
        g_dist = distributions.GibbsDAGDistributionFull(
            args.num_nodes, args.gibbs_temp, args.sparsity_factor)
    else:
        g_dist = distributions.GibbsUniformDAGDistribution(
            args.num_nodes, args.gibbs_temp, args.sparsity_factor)
    ...
    if not args.eval_only:
        for e in range(1, args.epochs + 1):
            temp_time = time.time()
            el, li, kl_g, baseline = train(
                model, bge_train, optimizer, baseline, args.batch_size, e, args.device)
            time_epoch.append(time.time() - temp_time)
            likelihood.append(li), kl_graph.append(kl_g), elbo_train.append(el)
            elbo_epoch, likelihood_epoch = evaluate(
                model, bge_train, args.batch_size, e, args.device)
            val_elbo.append(elbo_epoch)

            if e % 100 == 0:
                kl_full, hellinger_full = 0., 0.
                if args.num_nodes <= 4:
                    kl_full, hellinger_full = full_kl_and_hellinger(
                        model, bge_train, g_dist, args.device)
    ...
    with open(osp.join(args.save_path, 'results.pkl'), 'wb') as bb:
        pkl.dump({'likelihood': likelihood, 'kl_graph': kl_graph, 'elbo_train': elbo_train,
                  'elbo_val': val_elbo, 'kl_best_full': kl_full, 'hellinger_best_full': hellinger_full, 'time': time_epoch, 'baseline': baseline, 'exp_shd': shd, 'exp_prc': prc, 'exp_rec': rec, 'auroc': auroc_score}, bb)
